...
export class UserCourseModel {
  ...
  static async getUserCourses(userId) {
    const query = `
      ...
      LEFT JOIN user_courses uc ON s.subject_id = uc.subject_id AND uc.user_id = ?
      WHERE uc.user_id = ?
  ...
  static async enrollCourse(userId, subjectId) {
    const query = `
      INSERT INTO user_courses (user_id, subject_id, enrolled_at)
      VALUES (?, ?, NOW())
  ...
  static async unenrollCourse(userId, subjectId) {
    const query = `DELETE FROM user_courses WHERE user_id = ? AND subject_id = ?`;
    const [result] = await db.query(query, [userId, subjectId]);
    return result;
  }
  ...
  static async isEnrolled(userId, subjectId) {
    const query = `SELECT 1 FROM user_courses WHERE user_id = ? AND subject_id = ?`;
