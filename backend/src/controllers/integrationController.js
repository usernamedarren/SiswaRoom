...
export async function syncGrades(req, res) {
  try {
    ...
    const [students] = await db.query("SELECT id FROM users WHERE id = ? AND role = 'student'", [studentId]);
    ...
    await db.query(syncQuery, [studentId, courseId, grade, source || 'integration']);
